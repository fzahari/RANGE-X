...
def read_structure_from_directory( directory_path, selection_strategy, num_of_strutures ):
    ...
    with os.scandir(directory_path) as entries:
        for job in entries:
            if job.is_dir():
                vec_path = os.path.join(job.path, 'vec.txt')
                ener_path = os.path.join(job.path, 'energy.txt')
                # Skip failed evaluations (no output files written)
                if not (os.path.isfile(vec_path) and os.path.isfile(ener_path)):
                    continue
                try:
                    v = np.loadtxt(vec_path)
                    e = np.loadtxt(ener_path)
