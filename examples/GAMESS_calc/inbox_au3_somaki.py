# -*- coding: utf-8 -*-
"""
RANGE-GAMESS SF-TDDFT Conical Intersection Search for Au3

Au3 (gold trimer) has rich photochemistry:
...
Three individual Au atoms as fragments.
...
print("=" * 60)
print("RANGE-GAMESS SF-TDDFT CI Search: Au3")
print("  Three Au atoms -- exploring all geometries")
print("=" * 60)

# ============================================================
# Step 0: Create Au atom XYZ files
# ============================================================
print("\nStep 0: Creating Au atom files")
os.environ['KMP_DUPLICATE_LIB_OK'] = 'True'

for label in ['a', 'b', 'c']:
    fname = f'au_{label}.xyz'
    with open(fname, 'w') as f:
        f.write("1\n")
        f.write(f"Au atom {label}\n")
        f.write("Au   0.000000   0.000000   0.000000\n")
    print(f"  Created {fname}")

# Three Au atoms with box constraints:
# Au1: near origin (anchor)
# Au2: to the right (~2.5 A along +x)
# Au3: above (allows triangular, linear, all geometries)
# Boxes are wide enough for equilateral D3h, obtuse C2v, and linear D_inf_h
input_molecules = ['au_a.xyz', 'au_b.xyz', 'au_c.xyz']
input_num_of_molecules = [1, 1, 1]
input_constraint_type = ['in_box', 'in_box', 'in_box']
input_constraint_value = [
    (-0.3, -0.3, -0.2,  0.3,  0.3, 0.2),    # Au1: near origin
    ( 1.8, -0.8, -0.2,  3.2,  0.8, 0.2),    # Au2: ~2.5 A along +x
    (-0.5,  1.0, -0.2,  2.0,  3.0, 0.2),    # Au3: above, allows triangle
]

# ============================================================
# Step 1: Setting cluster model
# ============================================================
print("\nStep 1: Setting cluster model (three Au atoms)")
...
geo_opt_control_line = dict(
    method='GAMESS_SF_CI',
    input='input_gamess_au3_sf_tddft_template',
    alpha=0.02,
)
...
# Step 3: Run RANGE
# ============================================================
output_folder_name = 'results_au3_somaki'
