# -*- coding: utf-8 -*-
"""
RANGE-GAMESS SF-TDDFT Conical Intersection Search for Ag3

Ag3 (silver trimer) has rich photochemistry:
...
Three individual Ag atoms as fragments.
...
print("=" * 60)
print("RANGE-GAMESS SF-TDDFT CI Search: Ag3")
print("  Three Ag atoms -- exploring all geometries")
print("=" * 60)

# ============================================================
# Step 0: Create Ag atom XYZ files
# ============================================================
print("\nStep 0: Creating Ag atom files")
os.environ['KMP_DUPLICATE_LIB_OK'] = 'True'

for label in ['a', 'b', 'c']:
    fname = f'ag_{label}.xyz'
    with open(fname, 'w') as f:
        f.write("1\n")
        f.write(f"Ag atom {label}\n")
        f.write("Ag   0.000000   0.000000   0.000000\n")
    print(f"  Created {fname}")

# Three Ag atoms with box constraints:
# Ag1: near origin (anchor)
# Ag2: to the right (~2.5 A along +x)
# Ag3: above (allows triangular, linear, all geometries)
# Boxes are wide enough for equilateral D3h, obtuse C2v, and linear D_inf_h
input_molecules = ['ag_a.xyz', 'ag_b.xyz', 'ag_c.xyz']
input_num_of_molecules = [1, 1, 1]
input_constraint_type = ['in_box', 'in_box', 'in_box']
input_constraint_value = [
    (-0.3, -0.3, -0.2,  0.3,  0.3, 0.2),    # Ag1: near origin
    ( 1.8, -0.8, -0.2,  3.2,  0.8, 0.2),    # Ag2: ~2.5 A along +x
    (-0.5,  1.0, -0.2,  2.0,  3.0, 0.2),    # Ag3: above, allows triangle
]

# ============================================================
# Step 1: Setting cluster model
# ============================================================
print("\nStep 1: Setting cluster model (three Ag atoms)")
...
geo_opt_control_line = dict(
    method='GAMESS_SF_CI',
    input='input_gamess_ag3_sf_tddft_template',
    alpha=0.02,
)
...
# Step 3: Run RANGE
# ============================================================
output_folder_name = 'results_ag3_somaki'
