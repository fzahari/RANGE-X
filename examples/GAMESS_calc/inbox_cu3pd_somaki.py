# -*- coding: utf-8 -*-
"""
RANGE-GAMESS SF-TDDFT Conical Intersection Search for Cu3Pd

Cu3Pd: Bimetallic cluster -- Pd breaks D3h symmetry of Cu3.
SF-TDDFT from ROHF triplet reference -> singlet target.
SBKJC ECP for both Cu and Pd.

NOTE: MULT/ICHARG may need adjustment depending on the
total valence electron count with Pd.
  Cu: 19 valence e- (SBKJC) x 3 = 57
  Pd: 18 valence e- (SBKJC) x 1 = 18
  Total = 75 (odd) => MULT must be even (2, 4, ...)
  If the current MULT=3 fails, try:
    - ICHARG=1, MULT=3 (74 e-, even, triplet OK)
    - ICHARG=0, MULT=2 (75 e-, odd, doublet)
      with MULT=4 reference -> MULT=2 SF target

Fragments: 3 Cu atoms + 1 Pd atom (4 fragments)
RANGE explores all interatomic distances and orientations.

Science question: Does Pd doping lift or modify the
Jahn-Teller CI of Cu3? Connection to CO2->methanol catalysis.
"""
...
print("=" * 60)
print("RANGE-GAMESS SF-TDDFT CI Search: Cu3Pd")
print("  3 Cu + 1 Pd -- bimetallic cluster CI landscape")
print("=" * 60)
...
pd_xyz = 'pd_atom.xyz'
with open(pd_xyz, 'w') as f:
    f.write("1\n")
    f.write("Pd atom\n")
    f.write("Pd   0.000000   0.000000   0.000000\n")
print(f"  Created {pd_xyz}")

# 3 Cu atoms + 1 Pd atom
# Cu-Cu equilibrium ~2.3-2.5 A, Cu-Pd ~2.4-2.6 A
# Start with roughly tetrahedral / planar arrangements
# Let RANGE explore the full configuration space
input_molecules = [cu_xyz, pd_xyz]
input_num_of_molecules = [3, 1]
input_constraint_type = ['in_box', 'in_box']
input_constraint_value = [
    (-0.3, -0.3, -0.3,  0.3,  0.3, 0.3),   # Cu atoms near origin region
    ( 1.5, -1.5, -0.5,  3.5,  1.5, 0.5),    # Pd in a broad region
]

# NOTE: with 4 atoms, RANGE has more DOF than Cu3.
# The search space is larger -- may need more iterations.

print("\nStep 1: Setting cluster model (3 Cu + 1 Pd)")
...
geo_opt_control_line = dict(
    method='GAMESS_SF_CI',
    input='input_gamess_cu3pd_sf_tddft_template',
    alpha=0.02,
)
...
if results:
    ...
    print("  If MULT=3 fails (odd electron count), try:")
    print("    ICHARG=1, MULT=3  or  ICHARG=0, MULT=4->2 SF")
